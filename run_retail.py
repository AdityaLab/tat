...
from util.data import batch_sampled_data, batch_data, to_one_hot, RetailDataset, TimeSeriesDataLoader
...
def train(model, configs):
    ...
    x_train_inputs = np.load('./dataset/data/favorita/process_train_inputs.npy')
    # x_train_static = np.load('./dataset/data/electricity/process_train_identifier.npy', allow_pickle=True)

    x_valid_inputs = np.load('./dataset/data/favorita/process_valid_inputs.npy')
    # x_valid_static = np.load('./dataset/data/electricity/process_valid_identifier.npy', allow_pickle=True)

    x_test_inputs = np.load('./dataset/data/favorita/process_test_inputs.npy')
    ...
    train_set = RetailDataset(xt_train, horizon=configs.horizon, lookback=configs.lookback)
    vali_set  = RetailDataset(xt_valid, horizon=configs.horizon, lookback=configs.lookback)
    test_set  = RetailDataset(xt_test, horizon=configs.horizon, lookback=configs.lookback)
    ...
    for epoch in range(configs.epoch):
        ...
        early_stopping(vali_loss, model, path=root + '/retail_checkpoint/' + configs.model_name + '_checkpoint_{}.pth'.format(configs.seed))
    ...
    best_model_path =  root + '/retail_checkpoint/' + configs.model_name + '_checkpoint_{}.pth'.format(configs.seed)
...
def test(model, test_loader, criterion_50, criterion_90, device, configs, flag=False):
    ...
    if flag:
        np.save(root + '/retail_results/' + configs.model_name + '_pred_{}.npy'.format(configs.seed), preds)

    return total_loss

if __name__ == '__main__':

    parser = argparse.ArgumentParser(description='retail')

    # exp/data setup
    parser.add_argument('--seed', type=int, default=42, help='random seed')

    parser.add_argument('--lookback', type=int, default=90, help='lookback window size, past 6 months')
    parser.add_argument('--horizon', type=int, default=30, help='horizon window size, next 1 month')
    ...
    parser.add_argument('--num_xt', type=int, default=3, help='number of target time series')
    parser.add_argument('--num_xf', type=int, default=8, help='number of future time series')
    parser.add_argument('--num_xs', type=int, default=9, help='number of static classes')
    ...
    parser.add_argument('--hidden', type=int, default=240, help='hidden size')
    ...
    parser.add_argument('--position_global_atrous_rates', type=list, default=[1, 2, 4, 8, 14], help='positional encoding atrous rate')
    ...
    if not os.path.exists(root + '/retail_checkpoint'):
        os.mkdir(root + '/retail_checkpoint')

    if not os.path.exists(root + '/retail_results'):
        os.mkdir(root + '/retail_results')
